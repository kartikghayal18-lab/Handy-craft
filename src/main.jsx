...
function CategoryPage({ products, catalogState, retry, count, favourites, onFavourite, onView, onAdd, onCart }) {
  ...
  return <div className="category-page">
    ...
    <main>
      ...
      <section className="category-hero"><img src="/images/memory-kraft-hero.png" alt="Handmade personalized Forever Handy gift"/><div><p className="eyebrow">The gifting edit</p><h1>Memories, wrapped beautifully.</h1><p>Personalized keepsakes for the people who matter.</p><button className="primary" onClick={explore}>Shop now <Icon name="arrow"/></button></div></section>
...
function App() {
  ...
  const carouselItems = useMemo(() => catalogProducts.filter(product => product.image).slice(0, 5).map(product => ({ productId: product.id, tag: product.personalizable ? '#Personalized' : '#Bestseller', titleLine1: product.name, titleLine2: product.categories?.[0] ? `– ${product.categories[0]}` : '– FOREVER HANDY', desc: product.short_description || product.type || 'A personalized keepsake made from your favourite memories.', img: product.image, price: rupee(product.price), ctaText: product.personalizable ? 'Personalize' : 'View Gift' })), [catalogProducts]);
...
function Checkout({cart, close, complete}) {
  ...
  const place=async event=>{
    ...
    if(pendingVerification.current){
      ...
      catch(err){console.error('[checkout] verification retry failed',err);setError('Payment received. We are confirming your order. Please do not pay again.');}
    ...
    try{
      ...
      const checkout=new Razorpay({
        ...
        handler:async response=>{
          ...
          try{
            ...
            setError('Payment received. We are confirming your order. Please do not pay again.');
          }finally{paymentInProgress.current=false;setBusy(false);}
        },
      ...
      checkout.on('payment.failed',()=>{paymentInProgress.current=false;setBusy(false);setError('Payment received. We are confirming your order. Please do not pay again.');});
      checkout.open();
    }catch(err){
