...
import './commerce.css';
import Storefront from './v2/App.jsx';
...
function App() {
  ...
  // The storefront pages (src/v2) around the unchanged product popup, cart and checkout.
  const catalog = { ...catalogState, retry: loadCatalog };
  return <>
    <Storefront products={catalogProducts} catalogState={catalog} cartCount={count} onView={setSelected} onAdd={add} onCart={() => setDrawer(true)}/>
    <div className="hk-commerce">
      {cartNotice&&<CartSuccess notice={cartNotice} close={()=>setCartNotice(null)} viewCart={()=>{setCartNotice(null);setDrawer(true)}}/>}
      {selected && <ProductModal product={selected} close={() => setSelected(null)} add={(customization,quantity) => { add(selected, customization, quantity); setSelected(null); }}/>}
      {drawer && <CartDrawer cart={cart} total={total} close={() => setDrawer(false)} checkout={()=>{setDrawer(false);setCheckout(true)}} remove={remove} changeQuantity={changeQuantity}/>}
      {checkout && <Checkout cart={cart} close={()=>setCheckout(false)} complete={()=>{setCart([]);setCheckout(false)}}/>}
    </div>
  </>;
}
