...
export async function getOrderForNotification(orderId) {
  ...
  endpoint.searchParams.set(
    'select',
    'id,order_number,order_status,payment_status,total,email,shipping_name,shipping_address,shipping_city,shipping_state,shipping_postal_code,tracking_number,tracking_url,customer:customers(email,name),order_items(product_name_snapshot,quantity,price_snapshot)'
  );
